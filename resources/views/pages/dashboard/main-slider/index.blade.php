...
@section('content')
  ...
  <div class="container mt-5">
    ...
    <div class="col text-end">
      <a href="{{ route('admin.createmainslider') }}" id="add_slider" name="add_slider"
      class="btn btn-gray border float-right">Add Slider</a>
    </div>
    @endif
    </div>
    <div class="row">
    <div class="col-12 col-md-12">
      ...
      <thead>
        ...
        <th class="tbl_col3">{{ $settings->switch_slider == 1 ? 'Image' : 'Video' }}</th>
      ...
      <tbody>
        @php
      $sliderType = $settings->switch_slider == 1 ? 1 : 0;   
      @endphp
        @if(!empty($sliders) && $sliders->count())
        @foreach ($sliders as $slider)
        @if ($sliderType == $slider->switch)
      <tr class="action_area">
      ...
      <td>
        @if ($slider->banner)
      @if($settings->switch_slider == 1)
      <img src="{{ url('public/assets/frontend/img/sliders/' . $slider->banner) }}" alt="Slider Image"
      class="img-fluid" style="width: 100px; height: auto;" />
      @else
      <video src="{{ url('public/assets/frontend/img/video/' . $slider->banner) }}" width="300px" controls
      preload="none" muted>
      Your browser does not support the video tag.
      </video>
      @endif
      @else
      <h6>There have No any {{ $settings->switch_slider == 1 ? 'Image' : 'Video' }}</h6>
      @endif
      </td>
      <td></td>
      <td>
      ...
      <tfoot>
        <tr class="action_area">
        <th class="tbl_col1"><input type="checkbox" class="form-check-input" id="check_all_footer"
          name="check_all" /></th>
        <th class="tbl_col2">Title</th>
        <th class="tbl_col3">{{ $settings->switch_slider == 1 ? 'Image' : 'Video' }}</th>
        <th class="tbl_col4"><i class="fa fa-comment"></i></th>
