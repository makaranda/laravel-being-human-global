...
class NewsController extends Controller
{
    ...
    public function index()
    {
        $blogs = Blog::where('status', 1)
            ...
            ->get();
        return view('pages.dashboard.news.index', compact('blogs'));
    }
    ...
    {
        $blogs = Blog::all();
        return view('pages.dashboard.news.create', compact('blogs'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('file_input') && $request->file('file_input')->isValid()) {
            ...
            if ($file_input->move(public_path($filePath), $filename)) {
                ...
                $blog->feature_image = '';
                //return redirect()->route('admin.news')->with('error', 'Sorry, there was an error uploading your file.');
            }
        } else {
        ...
        return redirect()->route('admin.news')->with('success', 'News Blog created successfully');
    }
    ...
    public function edit($id)
    {
        ...
        // $blogs = Blog::all();
        $blogs = Blog::where('blog_type', 'news-events')->get();
        return view('pages.dashboard.news.edit', compact('blog', 'blogs'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('file_input')) {
            $filePath = 'public/assets/uploads/news/';
        ...
        // Redirect or return response
        return redirect()->route('admin.news')->with('success', 'News Blog updated successfully');
    }
    ...
    public function delete($id)
    {
        ...
        $blog = Blog::findOrFail($id);
        if (!$blog) {
            return redirect()->route('admin.news')->with('error', 'Blog not found.');
        }
        ...
        if (in_array($blog->slug, $defaultBlogs)) {
            return redirect()->route('admin.news')->with('error', 'You cannot delete the "' . $blog->title . '" Blog as it is a default Blog.');
        }
        // If the Blog has a feature image, delete it from storage

        // if ($blog->feature_image) {
        //     Storage::delete('public/assets/uploads/news/' . $blog->feature_image);
        // }
        if (!empty($blog->feature_image) && Storage::exists('public/assets/uploads/news/' . $blog->feature_image)) {
            Storage::delete('public/assets/uploads/news/' . $blog->feature_image);
        }
        ...
        return redirect()->route('admin.news')->with('success', 'News Blog status updated to inactive');
